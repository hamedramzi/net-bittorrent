{
...

    {
        ...
        sub new {
            ...
            my $self = undef;
            if (defined $args->{q[socket]} and defined $args->{q[client]}) {
                my (undef, $peerport, @address) =
                  unpack(q[SnC4x8], getpeername($args->{q[socket]}));

                # Constructor.
                $self = bless \sprintf(q[%d.%d.%d.%d:%d], @address, $peerport),
                  $class;
                ...
                $args->{q[session]}->peers)
            {
                ...
                ioctl($socket, 0x8004667e, pack(q[I], 1))
                  or carp(q[Failed to set socket blocking status]);

                setsockopt($socket, SOL_SOCKET, SO_SNDTIMEO, pack(q[LL], 30, 0))
                  or carp(q[Failed to set socket timeout]);
                setsockopt($socket, SOL_SOCKET, SO_RCVTIMEO, pack(q[LL], 30, 0))
                  or carp(q[Failed to set socket timeout]);
...
=cut
        ...
        sub incoming_connection  { return $incoming_connection{+shift}; }
        ...
        sub process_one {
            my $self  = shift;
            my $read  = shift;  # length (>= 0) we should read from this peer...
            my $write = shift;  # ...or write. In the future, this is how we'll
                                # limit bandwidth.
            ...
            if ($write and defined $socket{$self}) {
                ...
                if ($actual_write) {
                    $client{$self}->do_callback(q[peer_outgoing_data], $self,
                        $actual_write);
                }
            ...
            if ($read) {
                ...
                if ($actual_read) {
                    ...
                    if (not $connected{$self}) {
                        $connected{$self} = 1;
                        $client{$self}->do_callback(q[peer_connect], $self);
                    }
                    $client{$self}
                      ->do_callback(q[peer_incoming_data], $self, $actual_read);
        ...
        sub parse_reserved {
            ...
            # official extentions
            $extentions_FastPeers{$self} = (($reserved->[7] &= 0x04) ? 1 : 0);
            $extentions_DHT{$self}       = (($reserved->[7] &= 0x01) ? 1 : 0);

            # unofficial extentions
            $extentions_encryption{$self} = 0;    # TODO
            $extentions_PEX{$self}        = 0;    # TODO
            $extentions_protocol{$self} = (($reserved->[5] &= 0x10) ? 1 : 0);
        ...
        sub parse_packet {
            ...
            my (%ref, $type);
            if (unpack(q[c], $queue_incoming{$self}) eq 0x13) {
                ...
                ) = unpack(q[c/a a8 H40 a20 a*], $queue_incoming{$self});
...
qq[Improper handshake; Bad protocol name ($protocol_name)]
                ...
                if ($incoming_connection{$self}) {
                    my $session = $client{$self}->locate_session($info_hash);
                    if (defined $session
                        and $session->isa(q[Net::BitTorrent::Session]))
                    {
                        ...
                        {
                            return $self->disconnect(q[We have enough peers]);
                        }
                ...
                $client{$self}->do_callback(q[peer_incoming_handshake], $self);
                ...
                if ($type eq q[]) {
                    ...
                    $type = -1;
                    $client{$self}
                      ->do_callback(q[peer_incoming_keepalive], $self);
                }
                elsif ($type == 0) {
                    if ($packet_len != 1) {
                        $self->disconnect(q[Incorrect packet length for CHOKE]);
                        return;
                    }
                    $is_choking{$self}     = 1;
                    ...
                    $client{$self}->do_callback(q[peer_incoming_choke], $self);
                }
                elsif ($type == 1) {
                    if ($packet_len != 1) {
                        $self->disconnect(
                    ...
                    $is_choking{$self} = 0;
                    $next_pulse{$self} =
                      Net::BitTorrent::Util::min($next_pulse{$self}, time + 2);
                    $client{$self}
                      ->do_callback(q[peer_incoming_unchoke], $self);
                }
                elsif ($type == 2) {
                    if ($packet_len != 1) {
                        $self->disconnect(
                    ...
                    $client{$self}
                      ->do_callback(q[peer_incoming_interested], $self);
                }
                elsif ($type == 3) {
                    if ($packet_len != 1) {
                        $self->disconnect(
                    ...
                    $is_interested{$self} = 0;
                    $client{$self}
                      ->do_callback(q[peer_incoming_disinterested], $self);
                }
                elsif ($type == 4) {
                    if ($packet_len != 5) {
                        $self->disconnect(q[Incorrect packet length for HAVE]);
                        return;
                    }
                    ...
                    %ref = (index => $index);
                    $client{$self}
                      ->do_callback(q[peer_incoming_have], $self, $index);
                }
                elsif ($type == 5) {
                    ...
                    %ref = (bitfield => $packet);
                    $client{$self}
                      ->do_callback(q[peer_incoming_bitfield], $self);
                }
                elsif ($type == 6) {
                    if ($packet_len != 13) {
                        $self->disconnect(
                    ...
                    my $request = Net::BitTorrent::Peer::Request->new(
                        {   index  => $index,
                            offset => $offset,
                            length => $length,
                            peer   => $self
                    ...
                    $client{$self}
                      ->do_callback(q[peer_incoming_request], $self, $request);
                }
                elsif ($type == 7) {
                    if ($packet_len < 9) {
                        $self->disconnect(
...
q[Incorrect packet length for PIECE (%d requires >=9)],
                    ...
                    if (not $session{$self}->pieces->[$index]->working) {
                        $self->disconnect(q[Malformed PIECE packet. (1)]);
                    }
                    else {
                        my $block =
                          $session{$self}->pieces->[$index]->blocks->{$offset};
...
q[Peer sent us a piece we've already canceled or we never asked for.]
                            ...
                            $session{$self}->inc_downloaded(length $data);
                            $previous_incoming_block{$self} = time;
                            $block->piece->previous_incoming_block(time);

                            $client{$self}
                              ->do_callback(q[peer_incoming_block], $self,
                                $block);
                    ...
                    if (defined $request) {
                        $client{$self}
                          ->do_callback(q[peer_incoming_cancel], $self,
                            $request);
                    }
                    else {
                        $self->disconnect(
q[Peer has canceled a request they never made or has already been filled.]
                    ...
                    if ($packet_len < 3) {
                        $self->disconnect(q[Incorrect packet length for utx]);
                        return;
                    }
                    my ($messageid, $data) = unpack(q[ca*], $packet);
                    %ref = (
                        packet    => Net::BitTorrent::Util::bdecode($data),
                        messageid => $messageid,
                    );
                }
                else {
      ...
      # TODO: As we add support for more ext, this will be updated. For now, ...
        ...
        sub build_packet {
            ...
            if (not defined $packet->{q[type]}) {    # handshake
                ...
                $client{$self}->do_callback(q[peer_outgoing_handshake], $self);
            }
            elsif ($packet->{q[type]} == -1) {       # keepalive; no payload
                $packet_data = (pack(q[N], 0));
                $client{$self}->do_callback(q[peer_outgoing_keepalive], $self);
            }
            elsif (
                ($packet->{q[type]} == 0)            # choke
            ...
            {                                        # not interested
                ...
                $client{$self}->do_callback(
                    (
                          $packet->{q[type]} == 0 ? q[peer_outgoing_choke]
                        : $packet->{q[type]} == 1 ? q[peer_outgoing_unchoke]
                        : $packet->{q[type]} == 2 ? q[peer_outgoing_interested]
                ...
                $packet_data = pack(q[NCN],
                    5, $packet->{q[type]}, $packet->{q[data]}{q[index]});
                $client{$self}->do_callback(q[peer_outgoing_have], $self,
                    $packet->{q[data]}{q[index]});
            }
            elsif ($packet->{q[type]} == 5) {        # bitfield
                $packet_data = pack(q[NCa*],
                    length($packet->{q[data]}{q[bitfield]}) + 1,
                    $packet->{q[type]}, $packet->{q[data]}{q[bitfield]});
                $client{$self}->do_callback(q[peer_outgoing_bitfield], $self);
            }
            elsif (
                ($packet->{q[type]} == 6)            # request
                or ($packet->{q[type]} == 8)
              )
            {                                        # cancel
                my $packed = pack(q[N3],
                ...
                $packet_data = pack(q[NCa*],
                    length($packed) + 1,
                    $packet->{q[type]}, $packed);
                $client{$self}->do_callback(q[peer_outgoing_block], $self,
                    $packet->{q[data]}{q[request]});
            }
                ...
                or ($packet->{q[type]} == 15))
            {
                ...
                $packet_data = pack(q[NCN],
                    2, $packet->{q[type]}, $packet->{q[data]}{q[index]});
            }
...
q[Unknown packet! Please, include this in your bug report: %s],
        ...
        sub pulse {
            ...
            $self->unchoke if $is_interested{$self} and $is_choked{$self};

            if (scalar(@{$incoming_requests{$self}})) {
                while (
                    length($queue_outgoing{$self}) < $self->client->BufferSize
                    and my $request = shift @{$incoming_requests{$self}})
        ...
        sub request_block {
              ...
              )
            {
                ...
                if ($piece) {
                  REQUEST:
                    for (
                        scalar @{$outgoing_requests{$self}} .. $client{$self}
        ...
        sub disconnect {
            ...
            $client{$self}
              ->do_callback(q[peer_disconnect], $self, ($reason || $^E))
              if $connected{$self};
            return 1;
        }

        sub generate_fast_set { # http://www.bittorrent.org/fast_extensions.html
            my ($self, $k) = @_;
            my @a;
            $k ||= 9;
            ...
            my $x = sprintf(q[%X],
                (0xFFFFFF00 & (hex unpack q[H*], pack q[C*], $ip =~ m[(\d+)]g))
            );
            ...
            while (scalar @a < $k) {
                ...
                for (my $i = 0 ; $i < 5 && scalar @a < $k ; $i++) {
                    ...
                    push(@a, $index) unless grep { $_ == $index } @a;
                }
            }
            return $extentions_FastPeers_outgoing_fastset{$self} = \@a;
        }
