...
GetOptions(
    q[help|?]             => \$help,
    q[man]                => \$man,
    q[torrent|t=s@]       => \$dot_torrent,
    q[port|p:i]           => \$LocalPort,
    q[store|d|base_dir:s] => \$basedir
) or pod2usage(2);

if (not $dot_torrent and @ARGV) {
    push @$dot_torrent, shift @ARGV while (-e $ARGV[0]);
}

pod2usage(1) if $help or not $dot_torrent;
...
) or die sprintf q[Failed to create N::B object (%s)], $^E;
...
$SIG{q[INT]} = sub {
    ...
    print q[=] x 10
      ...
      . (q[=] x 10), qq[\n];
    print $client->as_string(1);
};

sub hashfail {
    my ($self, $piece) = @_;
    my $session = $piece->session;
   # printf(qq[on_hashfail: %04d|%s\n], $piece->index, $$session);
}

sub hashpass {
    my ($self, $piece) = @_;
    my $session = $piece->session;
    printf qq[on_hashpass: %04d|%s|%4d/%4d|%3.2f%%\n], $piece->index, $$session,
...
for my $dot_torrent (sort @$dot_torrent) {
    next if not -e $dot_torrent;
    printf qq[Loading '%s'...\n], $dot_torrent;
    my $session =
      $client->add_session({path => $dot_torrent, base_dir => $basedir})
      or warn sprintf q[Cannot load .torrent (%s): %s], $dot_torrent, $^E;
    $session->set_block_size(
        Net::BitTorrent::Util::max(
            2**14,
            (Net::BitTorrent::Util::min(($session->piece_size / 16), 2**15))
        )
    );
    $session->files->[2]->priority(0);
    $session->files->[1]->priority(0);
}
...
basic [options] [file ...]
 ...
 Options:
   ...
   -base_dir  base directory to store downloaded files
